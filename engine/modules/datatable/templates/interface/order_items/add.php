...
<form class="form" role="form" action="<?=$dt->baseUrl?>&act=<?if(is_numeric($dt->ent->getRowValue("id"))){?>update&id=<?=$dt->ent->getRowValue("id")?><?}else{?>add<?}?><?if($_GET["show"]=="folder_form")echo "&sp=folder"?>&show=<?=$_GET["show"]?>" method="post" enctype="multipart/form-data">
	...
	<div class="form__item" id="field_catalog_folder">
		...
		<div class="col-md-10">
			<?
			$sel = 0;
			if($dt->getRowValue('item_id')){
				$res = $db->query('select folder_id from catalog where id='.$dt->getRowValue('item_id'));
				if($t = $res->getNext())
					$sel = $t['folder_id'];
			}
	...
	<?foreach($fields as $k => $f)if(in_array($k, array('items_num'))){?>
...
<script type="text/javascript">
	...
	var dc = new doubleCombo("catalog_folder_id", document.forms[0].elements["item_id"], "/engine/ajax/getoptions.php", mpar, '<?=$dt->getRowValue('item_id')?>');
