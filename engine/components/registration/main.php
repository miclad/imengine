...
if(isset($_GET['success'])){
	...
	$GLOBALS["error_codes"][9] = 'Вы уже зарегистрированы на сайте, <a href="/forgot_password/">напомнить пароль?</a>';
	...
	if(isset($_POST['auto'])){
		...
		$GLOBALS['entities']['users'][1]['name'][2] = 1;
		$GLOBALS['entities']['users'][1]['name'][0] = 'Имя и фамилия';
		$GLOBALS['entities']['users'][1]['phone'][2] = 0;
		$GLOBALS['entities']['users'][1]['login'][1][1] = 'text';
		
		$dt_ob = new DataTable('users');
		
		$dt_ob->fields['login'][0] = 'Телефон';
	
		if(count($_POST)){
			
			$dt_ob->setRowValue('activated', 1);
			$dt_ob->setRowValue('group_id', 0);		
			$phone = CheckData::trimPhone($dt_ob->getRowValue('login'));
